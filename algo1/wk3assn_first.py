...
log_level = os.getenv("LOG_LEVEL", "CRITICAL") # TODO: make fix wrt this in some nice way
log_level = {"INFO": logging.INFO, "DEBUG": logging.DEBUG, "CRITICAL":logging.CRITICAL}.get(log_level)
...
def swap(arr, i, j):
    if i != j:
        ...
        arr[i] = arr[j]
        arr[j] = _temp
        LOGGER.debug(f"{arr} \t| swapped arr[{i}]={arr[j]} <--> arr[{j}]={arr[i]}: ")
...
def quicksort_and_count(arr: array.ArrayType, start: int, stop: int) -> int:
    LOGGER.debug(f"{arr} \t| Idxs: arr[{start:>2}:{stop+1: <2}] --> \t| {list(arr[start:stop+1])}")#\t arr id: {id(arr)}")
    if stop-start < 1:
        return 0
