...
def swap(arr, i, j):
    LOGGER.debug(f"swapping i={i}<-->j={j} ")#\t arr id: {id(arr)}")
    _temp = arr[i]
    arr[i] = arr[j]
    arr[j] = _temp
    LOGGER.debug(arr)
...
def quicksort_and_count(arr: array.ArrayType, start: int, stop: int) -> int:
    LOGGER.debug(f"start: {str(start):2s} \t stop: {str(stop):2s} \t arr: {arr} subarr: {arr[start:stop+1]}")#\t arr id: {id(arr)}")
    if stop-start <= 1:
        return 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--spec", help="", default="(3, 2, 4, 1,")
    ...
    LOGGER.info(f"Theoretical max (n**2) = {len(numbers)**2}")
    LOGGER.info(f"Theoretical min (n log n) = {len(numbers)*math.log2(len(numbers)):.2f}")
    LOGGER.debug(str(arr) + " <-- muted")
